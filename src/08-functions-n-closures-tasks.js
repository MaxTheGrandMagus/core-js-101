...
function getPolynom(...args) {
  ...
  return (y) => params.reduce((acc, val, index) => acc + val * y ** index, 0);
}
...
function logger(func, logFunc) {
  return (...args) => {
    const argStrArr = args.map((arg) => `${JSON.stringify(arg)}`);
    logFunc(`${func.name}(${argStrArr}) starts`);
    const result = func(...args);
    logFunc(`${func.name}(${argStrArr}) ends`);
    return result;
  };
}
