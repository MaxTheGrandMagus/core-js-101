...
function extractNameFromTemplate(value) {
  return value.match(/([A-Z])\w+/g).filter((item) => item !== 'Hello').join(' ');
}
...
function getRectangleString(width, height) {
  if (width < 2 || height < 2) {
    throw new Error('Imposible to create a rectangle');
  }
...
function getCardId(value) {
  ...
  return cards.indexOf(value) !== -1 ? cards.indexOf(value) : 'No matches';
}
